...
def assign_energy_three_param_tfcooperativity(mat, tf_e, nuc_e, delta_e_tf, enhancer_bounds):
    '''
    Function for assigning energies to each state in a data frame
    This is our "tf cooperativity" model, where the binding of 1 TF helps other TFs bind
    Here, delta_e_tf is a term that describes bonus energy per tf when other tfs are bound (means different things in different models)
    Model 1: bonus when >1 TF
    Model 2: numTFs choose 2 
    Model 3: optional cofactor
    '''
    num_tfs = mat.filter(like='tfbs_').sum(axis=1)
    num_tfs_choose2 = num_tfs * (num_tfs - 1) / 2
    num_nuc = pd.Series(0, index=mat.index)
    ...
    # # model 1
    # return num_tfs * (tf_e - delta_e_tf * (num_tfs>1)) + num_nuc * nuc_e

    # # model 2
    # return num_tfs * tf_e + num_tfs_choose2 * delta_e_tf + num_nuc * nuc_e 

    # model 3
    energy1 = num_tfs * tf_e + num_nuc * nuc_e
    energy2 = num_tfs * (tf_e + delta_e_tf) + num_nuc * nuc_e
    return -1 * np.log(np.exp(energy1) + np.exp(energy2))
...
def neg_log_like_multiple_tfcooperativity(x, *args):
    '''
    Likelihood function for computing how likely we were to see the data (observed state calls) given a specific energy (nuc, tf)
    x is (tf_e, nuc_e, (delta_e))
    # args is [[(molecules_1, states_1),...]]
...
#         enhancer_bounds = (240,550)
        ...
        # compute prob per state
        prob_per_state = convert_to_probabilities(assign_energy_three_param_tfcooperativity(states, tf_e, nuc_e, delta_e_tf, enhancer_bounds))
...
def fit_model(input_data, model):
    '''
    ...
    if model == '2param':
        starting_values = (-1, -1)
        likelihood = neg_log_like_multiple_two_param
    elif model == '3param_nuc':
        starting_values = (-1, -1, 0)
        likelihood = neg_log_like_multiple
    elif model == '3param_tfcoop':
        starting_values = (-1, -1, 0)
        likelihood = neg_log_like_multiple_tfcooperativity
    else:
        print('Not a valid model this should not happen')
        assert False

    res = minimize(likelihood, starting_values, args=input_data, method='Nelder-Mead')

    return res

def write_output(outfile, mle_output, model):
    if model == '2param':
        ...
        output.write('E_TF\t{}\n'.format(tf_e))
        output.write('E_nuc\t{}\n'.format(nuc_e))
        output.write('deltaE_tf_when_TF_bound\t{}\n'.format(delta_e))
    else:
        print('Not a valid model this should not happen')
        assert False
...
if __name__ == '__main__':
    ...
    with PdfPages(args.plot_file) as plots, open(args.output, 'w') as output:
        ...
        if args.model in valid_models:
            mle_params = fit_model(data, args.model)
        else:
            print('Invalid model string, please choose from: {}'.format(','.join(valid_models)))
            assert False

        # write to output file
        write_output(output, mle_params, args.model)
